import { ethers } from "hardhat";
import { sendTx } from "./tee/signer";

export async function deploySafeProxyFactory(deployerAddress: string): Promise<string> {
    const SafeProxyFactory = await ethers.getContractFactory("SafeProxyFactory");

    const deployTxData = await SafeProxyFactory.getDeployTransaction();
    let nonce = await ethers.provider.getTransactionCount(deployerAddress, "latest");

    const receipt = await sendTx(deployerAddress, {
        data: deployTxData.data,
        nonce: nonce,
    });
    nonce++;

    const createAddress = receipt.contractAddress;
    if (!createAddress) {
        throw new Error(`Contract not deployed. Receipt: ${JSON.stringify(receipt)}`);
    }
    console.log("SafeProxyFactory deployed to:", createAddress);

    return createAddress;
};